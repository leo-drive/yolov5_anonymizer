...
class yolov5onnx():
    def __init__(self, model_path, nc = 13, model_hw=[640,640], threshold=0.4, iou=0.5, nms_threshold=0.7, batch_size=1, mem_limit=800):
        ...
        ('CUDAExecutionProvider', {
            ...
            'gpu_mem_limit': mem_limit * 1024 * 1024,
    ...
    def post_processing(self, image_path, img):
        batch_detections = []
        count = 0
        img_hw = img.shape[:2]
        ...
        for i, box in enumerate(boxs):
            ...
            if (count < len(boxs)):
                ...
                # your new annotation file path
                yenilabel = open("/home/leo/yolo_ws/yolov5/deneme/"+filename + '.txt','a+')
                yenilabel.write(str(c) + ' ' + str(org_bboxes[0][1])
                                + ' '+ str(org_bboxes[0][2])
                                + ' ' + str(org_bboxes[0][3])
                                + ' ' + str(org_bboxes[0][4]) + '\n')
        ...
        return self._reverse_letterbox(bboxes_batch[0])

if __name__ == '__main__':

    # your image path
    images = glob("./deneme/*.jpg")
    for img_path in tqdm(images):
        ...
        im_hw = img.shape[:2] # original image size

        # your model path
        model_file = "/home/leo/yolo_ws/yolov5_anonymizer_ros/weights/face_lp.onnx"

        img_size = [640,640]

        om = yolov5onnx(model_file, nc=13, model_hw=img_size, mem_limit=800)
